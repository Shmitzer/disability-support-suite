...
// Turn the Caira character system on/off for the caller's whole org.
export async function saveCairaEnabled(enabled: boolean): Promise<SaveResult> {
  const worker = await getCurrentWorker();
  if (!worker) return { ok: false, error: "Not signed in." };
  if (!can(worker.role, Capability.OrgSettingsManage)) {
    return { ok: false, error: "You don't have permission to change settings." };
  ...
  try {
    await prisma.organisation.update({
      where: { id: worker.organisationId },
      data: { cairaEnabled: value },
    });
  } catch (err) {
    console.error("saveCairaEnabled failed:", err);
    return { ok: false, error: "Couldn't save — the settings column may not be set up yet." };
  }
  ...
  await recordAudit({
    ...
    actorId: worker.id,
    organisationId: worker.organisationId,
    detail: { cairaEnabled: value },
  });

  // The character chrome lives in the root layout, so refresh the whole tree.
