...
> - `Caira Site Guide.dc.html` — **DRAFT prototype** of the interactive marketing-site **guide host**: a large Caira stands on the left "waiting", with occasional waves/little jumps + soft synth blips (a ♪ puff cue) to signal she's interactive. **Click her** → she introduces herself (who she is / what she does) and offers a walk-through; **Yes** → she wanders the page in a calm-yet-playful way, stopping at each section (hero → how → who → pricing), each explained in a **responding pop-up text box** docked beside her (Next / Skip). **No voice — noises only.** Honours `prefers-reduced-motion` (no wander; bubble still steps through). Tweaks: `characterName`, `playSounds`, `autoGreet`, `idleGestureSeconds`. **Renderer note:** the character is the static `caira-master.png` cutout *as a stand-in* so the behaviour is demonstrable now; in production the `<img>` becomes `<CairaCharacter>` (the Rive rig) and the wander/tour logic here IS the renderer-agnostic **CairaWanderController** behaviour layer specced in `CAIRA_ANIMATION_RECONCILIATION.md`. To wire into the live sales site: lift the `.kyra`/`.kyra-bubble` markup + the `TOUR` script and point each stop's `data-tour` at the real sections.
