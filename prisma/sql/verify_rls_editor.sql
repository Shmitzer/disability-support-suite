...
BEGIN
  ...
  PERFORM set_config('request.jwt.claims',
    '{"sub":"00000000-0000-0000-0000-0000000000a1","organisationId":"verify_orgA"}', true);
  WITH upd AS (UPDATE public."Participant" SET name='hijacked' WHERE id='verify_pB' RETURNING 1)
  ...
  -- ---- Cleanup (as owner) ----
  PERFORM set_config('role', owner_role, true);
  DELETE FROM public."Participant"  WHERE id LIKE 'verify_%';
  DELETE FROM public."Organisation" WHERE id LIKE 'verify_%';
  RAISE NOTICE '============================================';
